...
const GCODE_REFERENCE: Record<string, { description: string; params?: Record<string, string> }> = {
  ...
  M400: {
    description:
      'Wait for moves to finish - ensures all previous commands complete before continuing',
  },
...
function getCommandInfo(
  command: string
): { description: string; params?: Record<string, string> } | null {
  // Direct match
                ...
                {parsed.params.map((param, idx) => {
                  ...
                  return (
                    <li key={idx} className="flex gap-2">
                      ...
                      {paramDesc && <span className="text-muted-foreground">— {paramDesc}</span>}
                    </li>
                  )
                })}
              </ul>
        ...
        {selectedLineInfo && (
          <div className="w-64 shrink-0 rounded-md border bg-background p-3">
            ...
            {selectedLineInfo.info ? (
              <>
                <p className="text-xs text-foreground mb-3">{selectedLineInfo.info.description}</p>
                      ...
                      {selectedLineInfo.parsed.params.map((param, idx) => {
                        ...
                        return (
                          <li key={idx} className="text-xs">
                            ...
                            <span className="font-mono text-foreground">{paramValue}</span>
                            {paramDesc && (
                              <div className="text-muted-foreground mt-0.5 pl-2">{paramDesc}</div>
                            )}
                          </li>
                        )
                      })}
      ...
      {showReference && (
        <div className="rounded-md border bg-muted/30 p-3 max-h-[300px] overflow-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {Object.entries(GCODE_REFERENCE)
              .slice(0, 30)
              .map(([cmd, info]) => (
